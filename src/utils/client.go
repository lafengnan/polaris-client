...
type MaintenanceOps interface {
	GetWarning(ch chan *http.Response) (err error)
}
...
/**Get the Maintenance warning
 * @ch the response channel
 */
func (c *PolarisClient) GetWarning(ch chan *http.Response) (err error) {

	method := "GET"
	url := "/polaris/maintenance/warning"
	host := "http://127.0.0.1"
	port := ":443"
	r, err := CallAPI(method, host+port+url, nil, nil)
	Perr(c.Logger, err, false)
	ch <- r
